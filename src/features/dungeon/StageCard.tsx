...
/**
 * **横置**的场景卡（dungeon 内唯一实现，开场房与战斗开局共用）。
 *
 * 为什么做成卡片：这一屏的视觉基调是卡片（下面是一排竖置的角色卡），而场景描述原本是一段裸文本，
 * 夹在卡片之间像"插入的一段说明"。横置卡 + 竖置角色卡也正好把两类东西分开：**横 = 场景 / 进度，
 * 竖 = 人**。
 *
 * **尺寸与角色卡（`ActorCard`）共用同一副足迹**（`--card-short` / `--card-tall`）：角色卡是
 * `短 × 长`（竖），场景卡是 `长 × 短`（横）——同一张卡转 90°。**固定大小是刻意的**：换房间、
 * 初始化前后都不该跳动。所以叙述超出就三行省略，**全文在浮窗里**（`StageInfoDialog`）。
 *
