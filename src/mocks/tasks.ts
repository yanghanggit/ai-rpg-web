 ...
 * 行为对齐真实后端（procrastinate）：任务从 `running` 开始，过一段时间自动 `completed`；
...
const tasks = new Map<string, { startedAt: number }>();
let sequence = 0;

/** 新建一个 mock 任务，返回其 job_id（后端用自增数字 id）。 */
export function createMockTask(now: number = Date.now()): string {
  sequence += 1;
