...
- **响应式的规则**（`.page` / `.page--wide` 的判据、卡片栅格写法）属于工程约定，见 [开发规范 §五](conventions.md)；本节只记**各页的排法**：家园页宽屏 3 列卡片，入口页宽屏左表单右蓝图，开发索引宽屏多列，副本页两段各自一行多张，副本房间页宽屏多列卡片（奖励候选）。
...
房间主体就是**准备**，对应 TUI 的 `/init` `/generate-spoils` `/pick-card` `/next`：`初始化开场 → 生成奖励 → 3 选 1 挑卡 → 进入下一关`。

- **按钮只出现「当前该做的那一步」**：未初始化时只有「初始化开场」，初始化后换成「生成奖励」；「进入下一关」常驻。
...
- **挑卡不做二次确认**：区标题写明「奖励 3 选 1，挑走一张后其余作废」，防误触靠卡片上的显式「挑选」按钮（与名单的「加入 / 移出」同一套心智）。领卡是**按成员各自算的**（每人一个 `SpoilsComponent`）：领完把该组件标记为 `claimed=true`（候选保留供回看），组件本身作为「已生成」守卫保留。
