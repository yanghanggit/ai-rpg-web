...
- **顶部动作区折叠成「副本操作」入口**：原三个按钮（副本信息 / 叙事 / 离开副本）收进 `RoomActionsDialog`（纵向列表，每项一行）。「副本信息」= `DungeonInfoDialog`（整体设定 / 房间 / 敌人 + **进度**）；「叙事」= `useNarrative` + `NarrativeOverlay`；「离开副本」= 回家园。**未读叙事信号上提到入口按钮**（变绿 + 角标），否则会被菜单吃掉。同一时刻只开一个浮窗：菜单 → 子浮窗是**切换**不是叠加（见「浮窗层级」）。**不提供「返回副本总览」**：按游戏逻辑，离开副本就是回家园，副本进行中也没有别的去处。
