...
| `/game/:userName/:gameName/dungeon/room` | `src/pages/DungeonRoomRoute.tsx` | **副本进行中（房间页）**：「进入副本」成功后切到这里——玩家的场景已变成副本第一关，总览页那一刻起已无事可做，而家园接口在副本进行中会被后端拒绝。`DungeonRoomRoute` 是**路由入口 + 房间类型解析器**：取回当前房间 `GET .../room`（含加载中 / 404 出口），再按服务端判别字段 `room.type` 把整页交给 `OpeningRoomPage`（开场）或 `CombatRoomPage`（战斗）——两个房间页各自成页、各自维护，共用 `RoomScaffold`（标题=副本名 (当前/总数) 房间名 + **标题行三个平级图标入口**：副本操作 / 副本信息 / 牌组，离开副本收在「副本操作」菜单里）。**刻意不给房间类型加路由**，路径始终是 `/dungeon/room`。详见「副本进行中（房间）」一节 |
...
- **一级**：角色信息、场景信息、蓝图信息、实体浏览器、道具管理、副本操作（副本房间标题行的齿轮）、副本信息（副本房间标题行的黑旗 / 副本卡片的 ⓘ）、进入副本确认、全部叙事、牌组、奖励（副本开场）、队伍牌组（副本房间的牌组入口）、进入下一关确认、登出确认。
...
- **标题行右侧三个平级图标入口**（都只看图标，靠 `aria-label` / `title` 说明）：
