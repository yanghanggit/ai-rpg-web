...
| `/game/:userName/:gameName/home` | `src/pages/HomeOverviewPage.tsx` | 家园概览：只有**功能按钮**与**场景卡片**两块。按钮条：`推进一步 · N 个角色`（人数直接写在按钮上）、`叙事 已看/总共`（右边大于左边即有新事件未看，点击弹出「全部叙事」浮层）、`← 返回上一级`（浮窗确认后登出）。卡片：每个 stage 一张，列出其中的 actor，底部有「切换到此场景」按钮；玩家当前所在卡片高亮并标记「当前所在」 |
