...
| `/game/:userName/:gameName/dungeon` | `src/pages/DungeonPage.tsx` | 副本页：由家园页工具栏的「副本」按钮切过来（独立一屏，可切回）。当前实现**队伍名单**（`PartyRosterComponent` 的 add / remove）：左/上为「当前队伍」（玩家 + 已选同伴，可移出），右/下为「可加入的同伴」（持 `NPCComponent` 且**排除玩家控制角色**的 NPC，可加入）；候选栏用**限高滚动列表**承载（NPC 很多时只在列表内部滚动，不用搜索框——见 [docs/conventions.md](docs/conventions.md)「交互基调」）；宽屏左右双栏、窄屏堆叠 |
